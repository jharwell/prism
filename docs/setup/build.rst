...
.. IMPORTANT:: If you want to build an optimized version of SILICON (necessary
...
Download ``scripts/bootstrap.sh`` BEFORE cloning the SILICON repo. The script
...
- 4th arg: The # of cores to use when building ARGoS/SILICON (should be set to
...
To build ``SILICON`` with optimizations (necessary for using ``SIERRA`` or
...
.. WARNING:: Whatever compiler you use to build SILICON should be the same
  compiler that you use to build ARGoS, otherwise ARGoS will be unable to load
  ``libsilicon.so``. Why? Because the ABI compatability documentation between
  ...
  SILICON with the other), depending on the versions used.
...
silicon repo followed by ``/build/docs/silicon/html/index.html``. For example,
if you built silicon under ``$HOME/research``, then you would do
``$HOME/research/silicon/build/docs/silicon/html/index.html`` in the address bar
of your browser.
...
you built silicon in ``$HOME/research``, do the following::

  cd $HOME/research/silicon/build/docs/silicon/latex
  make
...
#. Verifying that SILICON, COSM, RCPPSW are all on the ``devel`` branch.

#. Updating RCPPSW, COSM, SILICON to the latest ``devel`` branch via ``git
   pull``.

#. Updating the SILICON, COSM, RCPPSW cmake submodules by running::
