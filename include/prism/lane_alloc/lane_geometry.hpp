...
class lane_geometry : public rer::client<lane_geometry> {
 public:
  lane_geometry(const pcperception::ct_skel_info* target,
                const pgrepr::ct_coord& ingress_cell,
                const pgrepr::ct_coord& egress_cell);
  ...
  const pgrepr::ct_coord& ingress_virt(void) const { return mc_ingress_virt; }
  const pgrepr::ct_coord& egress_virt(void) const { return mc_egress_virt; }
 ...
 private:
  /* clang-format off */
  const pgrepr::ct_coord  mc_ingress_virt;
  const pgrepr::ct_coord  mc_egress_virt;
