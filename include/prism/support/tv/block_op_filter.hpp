...
#ifndef INCLUDE_PRISM_SUPPORT_TV_BLOCK_OP_FILTER_HPP_
#define INCLUDE_PRISM_SUPPORT_TV_BLOCK_OP_FILTER_HPP_
...
class block_op_filter : public rer::client<block_op_filter> {
 public:
  explicit block_op_filter(const carena::base_arena_map* const map)
      : ER_CLIENT_INIT("prism.support.tv.block_op_filter"),
        mc_map(map) {}
 ...
 private:
  ...
  op_filter_status free_pickup_filter(const TControllerType& controller) const {
     ...
     * base controller for PRISM yet, so I can't create a
...
#endif /* INCLUDE_PRISM_SUPPORT_TV_BLOCK_OP_FILTER_HPP_ */
