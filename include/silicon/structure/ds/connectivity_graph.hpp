...
#ifndef INCLUDE_SILICON_STRUCTURE_DS_CONNECTIVITY_GRAPH_HPP_
#define INCLUDE_SILICON_STRUCTURE_DS_CONNECTIVITY_GRAPH_HPP_
 ...
 * structure3D to be built, read from a file during initialization.
 */
class connectivity_graph : public rer::client<connectivity_graph>,
 ...
 public:
  ...
  ssrepr::block_anchor_spec* to_spec(const boost::optional<vertex_descriptor>& vd) {
    if (vd) {
      return to_spec(*vd);
    }
    return nullptr;
  ...
  const ssrepr::block_anchor_spec* to_spec(const boost::optional<vertex_descriptor>& vd) const {
    return (const_cast<connectivity_graph*>(this))->to_spec(vd);
  }

  ssrepr::block_anchor_spec* to_spec(const vertex_descriptor& vd) { return &decoratee()[vd]; }
  const ssrepr::block_anchor_spec* to_spec(const vertex_descriptor& vd) const {
    return &decoratee()[vd];
  }
...
#endif /* INCLUDE_SILICON_STRUCTURE_DS_CONNECTIVITY_GRAPH_HPP_ */
