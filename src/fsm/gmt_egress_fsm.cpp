    ...
    : builder_util_fsm(params, rng, fsm_state::ekST_MAX_STATES),
      ...
      RCPPSW_HFSM_DEFINE_STATE_MAP(
          ...
          RCPPSW_HFSM_STATE_MAP_ENTRY_EX_ALL(&acquire_egress_lane,
                                             ...
                                             nullptr),
          RCPPSW_HFSM_STATE_MAP_ENTRY_EX_ALL(&gmt_egress,
                                             nullptr,
                                             &entry_gmt_egress,
                                             nullptr),
 ...
 * States
 ******************************************************************************/
RCPPSW_HFSM_STATE_DEFINE_ND(gmt_egress_fsm, start) {
  return rpfsm::event_signal::ekHANDLED;
}

RCPPSW_HFSM_STATE_DEFINE_ND(gmt_egress_fsm, acquire_egress_lane) {
  auto prox_checker = perception()->builder_prox();
  auto prox =
      prox_checker->operator()(allocated_lane(), prepr::fs_configuration::ekNONE);

  if (pcperception::builder_prox_type::ekNONE != prox) {
...
RCPPSW_HFSM_ENTRY_DEFINE_ND(gmt_egress_fsm, entry_acquire_egress_lane) {
  /*
...
RCPPSW_HFSM_ENTRY_DEFINE_ND(gmt_egress_fsm, entry_gmt_egress) {
  /*
...
RCPPSW_HFSM_ENTRY_DEFINE_ND(gmt_egress_fsm, entry_finished) {
  /*
 ...
 * Taskable Interface
 ******************************************************************************/
void gmt_egress_fsm::task_start(cta::taskable_argument* c_arg) {
...
void gmt_egress_fsm::task_execute(void) {
  if (event_data_hold()) {
